  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="fixed inset-0 z-50 bg-[#07070b] flex flex-col justify-between px-4 sm:px-6 pt-[max(1rem,env(safe-area-inset-top,1rem))] pb-[max(1.25rem,env(safe-area-inset-bottom,1.25rem))] select-none overflow-hidden touch-none"
      >
        ...
        <div className="flex-1 my-3 overflow-hidden flex flex-col justify-center">
          ...
          {activeTab === 'player' && (
            ...
            >
              <div className="relative w-56 h-56 sm:w-72 sm:h-72 md:w-80 md:h-80 rounded-3xl overflow-hidden shadow-[0_24px_64px_rgba(0,0,0,0.85)] border border-white/[0.12] group">
                <img
          ...
          {activeTab === 'lyrics' && (
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto space-y-4 py-4 px-2 text-center" dir="auto">
                {lyrics.length > 0 ? (
                  lyrics.map((l, idx) => {
                    ...
                    return (
                      <p
                        key={idx}
                        onClick={() => seek(l.time)}
                        className={`cursor-pointer transition-all duration-300 py-1 font-bold ${
                          isActive
                            ? 'text-white text-lg sm:text-xl drop-shadow-[0_0_15px_rgba(255,255,255,0.7)] scale-105'
                            : 'text-zinc-500 hover:text-zinc-300 text-sm sm:text-base'
                        }`}
                      >
                        {l.text}
                      </p>
